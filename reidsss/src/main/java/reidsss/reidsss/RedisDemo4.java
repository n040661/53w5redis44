 ...
 *
 */
public class RedisDemo4 implements Runnable {
	...
	private static void getIps() {
		...
		for (String ip : ips) {
			queryScanResult(jedis, ip, ipInterfaceIds, resultValues, "0");
		}
	...
	private static void queryScanResult(Jedis jedis, String ip, List<String> ipInterfaceIds, List<TestvO> resultValues,
		...
		// TODO
		// jedis.select(0);
		// queryListInfo(jedis, ipInterfaceIds, resultValues);
		...
		if (!"0".equals(hscanResult.getStringCursor())) {
			...
			queryScanResult(jedis, ip, ipInterfaceIds, resultValues, cursor);
		}
	...
	private static void queryListInfo(Jedis jedis, List<String> ipInterfaceIds, List<TestvO> resultValues) {
		...
		// handlerDatas(resultValues, responses);
	}

	private static void handlerDatas(List<TestvO> resultValues, Map<String, Response<List<String>>> responses) {

		Collection<Response<List<String>>> values = responses.values();

		for (Response<List<String>> valus : values) {
			List<String> list = valus.get();
			if (null == list) {
				continue;
			}
			int size = list.size();
