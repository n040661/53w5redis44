...
public class RedisDemoJSON {
	...
	private static void hash2() throws ParseException {
		...
		String a = "{caps : 34343333.00,sucReest:3444333.00,quest:5555.0000,ttool:8888.000,tcaps:9999444.000,hostname:hstom,itemName:afasjflafjajafj}";
		...
		for (String year : years) {
			...
			for (; iter.hasNext();) {
				...
				values = iter.next();
				jedis.hset(values.getKey(), year, values.getValue());
	...
	private static void init() {
		...
		jedisPool = new JedisPool(config, "localhost", 6379, 90000000, "foobared", 0); // 配置、ip、端口、连接超时时间、密码、数据库编号（0~15）
	}
