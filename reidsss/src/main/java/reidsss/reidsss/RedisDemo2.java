...
public class RedisDemo2 {
	...
	private static void hash2() throws ParseException {
		...
		String a = "34343333.00;3444333.00;5555.0000;8888.000;9999444.00";

		Map<Long, String> infos = new HashMap<>();
		// interceId
		for (int i = 0; i < 50; i++) {
			for (int j = 0; j < 500; j++) {
				infos.put(Base62.decode(i + "" + j), a);
			}
		}
		...
		Iterator<Entry<Long, String>> iter = null;
		Entry<Long, String> values = null;
	...
	private static void init() {
		...
		jedisPool = new JedisPool(config, "localhost", 6379, 90000000, "beijing", 6); // 配置、ip、端口、连接超时时间、密码、数据库编号（0~15）
	}
