...
public class RedisDemo implements Runnable {
	...
	private static void queryListInfo(Jedis jedis, List<String> ipInterfaceIds, List<TestvO> resultValues,
		...
		Map<String, Response<String>> responses = new HashMap<>(ipInterfaceIds.size());
	...
	private static void hash2() throws ParseException {
		...
		for (String year : years) {
			...
			for (int i = 0; iter.hasNext(); i++) {
				...
				// map.put(values.getValue(), Double.valueOf(year));
				// jedis.zadd("game".getBytes(), user1.getScore(),
				// ObjectSer.ObjectToByte(user1));
				jedis.lpush(values.getKey(), year + "," + values.getValue());
				// map.clear();
	...
	private static String[] get90Days() {
		List<String> years = getYears();
		List<String> subList = years.subList(0, 1);
